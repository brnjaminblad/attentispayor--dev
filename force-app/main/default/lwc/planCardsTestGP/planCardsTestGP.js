...
export default class PlanCards extends OmniscriptBaseMixin(LightningElement) {
    ...
    connectedCallback() {
        ...
        //console.log(JSON.stringify(this.plansOrdered[2]));
    ...
    openModal(event) {
        let cardid = event.target.dataset.id;
        this.product = this.plansOrdered[cardid];
        // this.attArray.push({Id:'1', Name: 'locooo'});
        // this.attArray.push({Id:'2', Name: 'madddd'});


        //console.log(JSON.stringify(this.product));

        if (this.product.attributeCategories) {
            for (let a = 0; a < this.product.attributeCategories.records.length; a++) {
                //console.log("plan:", i, "-", "AttCat record:", a, this.product.attributeCategories.records[a]);
                for (let b = 0; b < this.product.attributeCategories.records[a].productAttributes.records.length; b++) {
                    //console.log("plan:", i, "-", "AttCat record:", a, "-", "ProAtt record:", b, this.product.attributeCategories.records[a].productAttributes.records[b]);

                    //console.log('ori label: ' + this.product.attributeCategories.records[a].productAttributes.records[b].label);

                    // let name = this.product.attributeCategories.records[a].productAttributes.records[b].label.replace(/ /g, "");
                    let name = this.product.attributeCategories.records[a].productAttributes.records[b].label;
                    //name = name.replace(/-/g, "");
                    let value = this.product.attributeCategories.records[a].productAttributes.records[b].userValues;
                    //console.log(name, "=", value);
                    //this.newPlans[i][name] = value;

                    if(value != null){
                        if(name != 'Product Code' && name != 'Plan Type'){
                            if(value.includes('http') || value.includes('www')){
                                this.attArray.push({Name: name, Value: value, Web:true});
                            }else{
                                this.attArray.push({Name: name, Value: value, Web:false});
                            }
        ...
        if (this.product.childProducts) {
            for (let a = 0; a < this.product.childProducts.records.length; a++) {
                for (let b = 0; b < this.product.childProducts.records[a].attributeCategories.records.length; b++) {
                    for (let c = 0; c < this.product.childProducts.records[a].attributeCategories.records[b].productAttributes.records.length; c++) {

                        //console.log("plan:", i, "-", "childProducts record:", a, "-", "attributeCategories record:", b, "productAttributes record:", c, this.product.childProducts.records[a].attributeCategories.records[b].productAttributes.records[c]);
                        //console.log('antes name');
                        //let name = this.product.childProducts.records[a].attributeCategories.records[b].productAttributes.records[c].label.replace(/ /g, "");
                        let name = this.product.childProducts.records[a].attributeCategories.records[b].productAttributes.records[c].label;
                        //console.log('despues name ' + name);

                        //let code = this.product.childProducts.records[a].ProductCode;
                        // name = name.concat(code);
                        // name = name.replace(/-/g, "");
                        // name = name.replace("(", "");
                        // name = name.replace(")", "");
                        ...
                        let value = this.product.childProducts.records[a].attributeCategories.records[b].productAttributes.records[c].userValues;
                        ...
                        // if(value != null){
                        //     if(value.includes('http') || value.includes('www')){
                        //         this.covArray.push({Name: name, Value: value, Web:true});
                        //     }else{
                        //         this.covArray.push({Name: name, Value: value, Web:false});
                        //     }
                        // }else{
                        //     this.covArray.push({Name: name, Value: 'N/A', Web:false});
                        // }
                            
                            
                        //}

                        this.covArray.push({Name: name, Value:value});
                    }
                }
            }
        }
        ...
        this.prodParam = {Product: this.product, Lista:this.attArray, Coverages: this.covArray};

        const modal = this.template.querySelector("c-plan-modal-test-g-p");
        modal.show();
    }
    ...
    changeView() {
        ...
        if (this.countPlansToQuote > 0) {
            this.countPlansToQuote = 0;
            for (let i = 0; i < this.plansOrdered.length; i++) {
                if (changeViewArray[i]['checkboxQuote'] == 'true') {
                    changeViewArray[i]['checkboxQuote'] == 'false'
                    changeViewArray[i]['cardBorder'] = 'plancard';
                    console.log('selected for quote:', i, changeViewArray[i]['checkboxQuote']);
        ...
        /*if(this.viewState==false){
            this.template.querySelector(".compare-modal-btn").disabled;
        }*/
